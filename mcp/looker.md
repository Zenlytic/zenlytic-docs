---
description: >-
  Connect Zoë to Looker's MCP server to query semantic models, dashboards, and
  metadata from chat.
---
...
* **Authentication** — **Require authentication**. Zenlytic signs requests with a Google identity token; see [Configure request headers](looker.md#configure-request-headers).
...
3. Add the `Authorization` header (see [Configure request headers](looker.md#configure-request-headers)).
