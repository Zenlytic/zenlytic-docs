...
{% hint style="info" %}
**Folder access controls who can open an artifact, not what data they see inside it.** Dashboards, charts, and other HTML artifacts use [Live Queries](artifacts.md#live-queries) — those queries re-run when someone opens the artifact, using that person's own data permissions. Granting folder access never widens anyone's data access, so two people with the same folder access may still see different numbers in the same artifact.
{% endhint %}
