---
description: >-
  Connect Zoë to Jira's MCP server to search issues, run JQL, and work with
  Atlassian content.
---
...
> Zenlytic's per-user OAuth connectors are available only for [Google Workspace](google-workspace.md) and [SharePoint](sharepoint.md) at this time. Atlassian connections use the API token path described here. API token authentication must be enabled by an Atlassian organization admin before it can be used.
...
2. Fill out the form:
   ...
   *   **URL** — the Atlassian Rovo MCP endpoint:
       ...
       ```
3. Add the `Authorization` header (see [Configure request headers](jira.md#configure-request-headers)).
...
* **No OAuth for Atlassian.** Zenlytic's OAuth connectors cover Google Workspace and SharePoint only at this time. The API token Basic auth path described here is the only supported authentication method for Atlassian.
