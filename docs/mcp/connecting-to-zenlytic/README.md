...
{% hint style="info" %}
If your org logs in at a custom vanity subdomain (e.g. `acme.zenlytic.com`), use that subdomain in front of the MCP host instead: `https://acme.mcp.zenlytic.com/mcp`. The exact URL for your org is shown in **Settings → MCP** (**Workspace Settings → MCP** on workspaces without the new Settings experience yet).
{% endhint %}
...
1. In Zenlytic, go to **Settings → Profile → Personal Access Tokens** (`/personal-access-tokens`; **Workspace Settings → Personal Access Tokens** on workspaces without the new Settings experience yet).
