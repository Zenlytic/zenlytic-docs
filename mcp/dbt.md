---
description: >-
  Connect Zoë to dbt's MCP server to explore models, metrics, exposures, and
  lineage from chat.
---
...
* Run ad-hoc SQL against your warehouse with `execute_sql` (PAT-only — see [Choose a token type](dbt.md#choose-a-token-type)).
...
3. Add the required headers (see [Configure request headers](dbt.md#configure-request-headers)).
