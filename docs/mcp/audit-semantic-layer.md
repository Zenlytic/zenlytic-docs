...
Follow the [GitHub MCP setup guide](client/github.md) to mint a fine-grained PAT scoped to the data-model repository and add the connection in **Settings → MCP**. For an audit, the **default toolset** (`context`, `repos`, `issues`, `pull_requests`, `users`) is enough — Zoë only needs read access to repository contents.
