---
description: >-
  Connect Zoë to GitHub's MCP server to browse repos, issues, pull requests, and
  Actions from chat.
---
...
3. Add the `Authorization` header (see [Configure request headers](github.md#configure-request-headers)).
4. Optionally restrict the toolset (see [Customize the toolset](github.md#customize-the-toolset)).
...
* **No OAuth for GitHub.** The PAT path described here is the supported authentication method for GitHub.
