...
Access control is managed in Zenlytic through two concepts. The first, [access grants](../5_data_modeling/8_access_grants.md#access-grants), cover *column-based* access control, and the second, [access filters](../5_data_modeling/8_access_grants.md#access-filters), cover *row-based* access control.

## Access grants

Access grants are restrictions for certain users on the ability to see various fields and query them in the Zenlytic interface. These access restrictions are based on access to *columns* and *views*. For row-based access control look at [access filters](../5_data_modeling/8_access_grants.md#access-filters).
...
User attributes are strings that can handle [filter syntax](../5_data_modeling/94_field_filter.md) for specifying complex comparisons or inclusions in either access grants (column level security) or access filters (row level security).
