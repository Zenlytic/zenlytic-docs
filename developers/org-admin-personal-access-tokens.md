# Personal access tokens (org admin)

Personal access tokens (PATs) authenticate API requests to Zenlytic on behalf of your user account, without requiring an interactive login for each request. Use a PAT for scripts, CI pipelines, and other automated access to org admin endpoints.

## What is a personal access token?

A personal access token is a long-lived credential tied to your user account. Use it to authenticate API requests in place of a session login.

- **Scope:** Org-wide. Authenticates requests to `/workspace-manager/...` endpoints across every workspace in your organization.
- **Access:** Has the same permissions as your user account, which requires the Org Admin role for these endpoints.
...
The same permissions your user account has, including the Org Admin role required for org admin endpoints. A PAT does not grant elevated access beyond what you can already do when logged in.
