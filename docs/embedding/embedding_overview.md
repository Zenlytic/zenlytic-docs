...
* Signed embedding lets you provide a seamless authentication experience for your end customers. In this method, you use signed-embedding client credentials (a `client_id` and `client_secret` provisioned by your Zenlytic representative) to hit an endpoint, which returns a signed URL for the embedded content you've selected to display to the user. These are not the same as personal access tokens or workspace API keys. See [Signed Embedding](signed_embedding.md#credentials).
