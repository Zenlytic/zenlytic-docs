---
description: >-
  Reference the legacy dashboard file format and related concepts still
  supported in existing projects.
layout:
  width: default
  title:
    visible: true
...
* [Artifacts](../../zenlytic-ui/artifacts.md) for creating, sharing, rebuilding, and delivering modern dashboard-style outputs.
* [Data Modeling Overview](../../data-modeling/data_modeling.md) for the core model structure behind dashboards.
* [Field Filters](../../data-modeling/field_filter.md) for the filter syntax used by dashboard and element filters.

## Dashboard Properties

`type`: (Required) The type of the file. For these dashboard files is should always be `dashboard`.

`name`: (Required) The name of the dashboard. This name is used to uniquely determine the dashboard and presented in the url, so it must be unique across dashboards in your project. If you reference this dashboard elsewhere this is the name you will use. Like all names, it follows [Zenlytic naming conventions](../../data-modeling/data_modeling.md#naming-conventions)
...
`filters`: This is a list of filters which follow the [field filter](../../data-modeling/field_filter.md) syntax.
...
`model`: (Required) The name of the [model](../../data-modeling/model.md) you want to base this dashboard element from.

`metrics`: (Required) This is a list of [measure](../../data-modeling/measure.md) names to display in the dashboard element. For example, `orders.total_revenue` would reference the `total_revenue` measure in the `orders` view.

`slice_by`: This is a list of slices ([dimensions](../../data-modeling/dimension.md) or [dimension groups](../../data-modeling/dimension_group.md)) to apply to the plot or table. For example, `orders.new_vs_repeat` references the `new_vs_repeat` dimension in the `orders` view, and `orders.order_month` references the dimension\_group `order` using the `month` timeframe.

`filters`: This is a list of filters that follow the standard [field filter](../../data-modeling/field_filter.md) syntax. For example, the following filter ensures that the `product_name` dimension in the `order_lines` view is not equal to "Handbag"
...
a datetime [field filter](../../data-modeling/field_filter.md#dates).\
For more advanced, fine-grained controls over the time filters, use the [field filters](../../data-modeling/field_filter.md) under the `filters` property.
